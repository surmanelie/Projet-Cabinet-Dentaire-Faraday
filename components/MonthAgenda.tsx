import Link from "next/link";

const MONTHS = [
  "Janvier", "Février", "Mars", "Avril", "Mai", "Juin",
  "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre",
];
const WEEKDAYS = ["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"];

export type DayTemplate = { startTime: string; endTime: string };
export type DayEntry = {
  source: string;
  actualStart: string | null;
  actualEnd: string | null;
  comment: string | null;
  plannedStart?: string | null;
  plannedEnd?: string | null;
  locked?: boolean;
};

function pad(n: number) {
  return String(n).padStart(2, "0");
}

/**
 * Agenda mensuel en grand : une case par jour, avec l'horaire prévu (override
 * du jour, sinon semaine type), les congés, et les heures réellement
 * pointées. Navigation mois par mois. Reste un composant d'affichage pur par
 * défaut ; en passant `selectable`, l'appelant (SelectablePlanningCalendar)
 * peut activer le clic sur les jours pour une sélection multiple, utilisée
 * pour l'édition en masse côté admin — l'espace assistante l'utilise
 * toujours en lecture seule.
 */
export default function MonthAgenda({
  year,
  month,
  todayKey,
  templatesByDow,
  entriesByDate,
  prevHref,
  nextHref,
  selectable = false,
  selectedDates,
  onToggleDate,
}: {
  year: number;
  month: number;
  todayKey: string;
  templatesByDow: Record<number, DayTemplate>;
  entriesByDate: Record<string, DayEntry>;
  prevHref: string;
  nextHref: string;
  selectable?: boolean;
  selectedDates?: Set<string>;
  onToggleDate?: (key: string) => void;
}) {
  const firstDay = new Date(year, month - 1, 1);
  const offset = (firstDay.getDay() + 6) % 7;
  const daysInMonth = new Date(year, month, 0).getDate();

  const cells: (number | null)[] = [];
  for (let i = 0; i < offset; i++) cells.push(null);
  for (let d = 1; d <= daysInMonth; d++) cells.push(d);
  while (cells.length % 7 !== 0) cells.push(null);

  return (
    <div className="card p-4 sm:p-6">
      <div className="mb-5 flex items-center justify-between">
        <Link href={prevHref} className="btn-secondary px-3 py-1.5 text-sm" aria-label="Mois précédent">←</Link>
        <h2 className="section-title">{MONTHS[month - 1]} {year}</h2>
        <Link href={nextHref} className="btn-secondary px-3 py-1.5 text-sm" aria-label="Mois suivant">→</Link>
      </div>

      <div className="grid grid-cols-7 gap-1.5 sm:gap-2.5">
        {WEEKDAYS.map((w) => (
          <div key={w} className="pb-1 text-center text-xs font-medium uppercase tracking-wide text-ardoise-400">
            {w}
          </div>
        ))}

        {cells.map((day, i) => {
          if (day === null) return <div key={`e${i}`} className="min-h-[92px] rounded-xl bg-transparent sm:min-h-[130px]" />;

          const dow = new Date(year, month - 1, day).getDay();
          const key = `${year}-${pad(month)}-${pad(day)}`;
          const entry = entriesByDate[key];
          const tpl = templatesByDow[dow];
          const isToday = key === todayKey;
          const isAbsence = entry?.source === "absence";
          const plannedStart = entry?.plannedStart ?? tpl?.startTime ?? null;
          const plannedEnd = entry?.plannedEnd ?? tpl?.endTime ?? null;
          const isSelected = selectedDates?.has(key) ?? false;
          const isLocked = Boolean(entry?.locked);

          const cellClasses = [
            "min-h-[92px] rounded-xl border p-1.5 text-left transition-all duration-200 ease-premium sm:min-h-[130px] sm:p-2.5",
            isSelected
              ? "border-faraday-600 bg-faraday-50 ring-2 ring-faraday-500"
              : isToday
                ? "border-faraday-500 bg-faraday-50"
                : "border-ardoise-100 bg-white",
            selectable && !isLocked ? "cursor-pointer hover:border-faraday-400" : "",
            selectable && isLocked ? "cursor-not-allowed opacity-60" : "",
          ].join(" ");

          const content = (
            <>
              <div className="flex items-start justify-between">
                <span className={`text-xs font-medium sm:text-sm ${isToday ? "text-faraday-700" : "text-ardoise-500"}`}>{day}</span>
                {selectable && (
                  <span
                    aria-hidden="true"
                    className={`flex h-4 w-4 items-center justify-center rounded-full border text-[10px] ${
                      isSelected ? "border-faraday-600 bg-faraday-600 text-white" : "border-ardoise-300 text-transparent"
                    }`}
                  >
                    ✓
                  </span>
                )}
                {isLocked && <span aria-hidden="true" className="text-xs text-ardoise-400">🔒</span>}
              </div>

              {isAbsence ? (
                <div className="mt-1.5 rounded-md bg-amber-50 px-1.5 py-1 text-center text-[11px] font-medium text-amber-700 sm:text-xs">
                  {entry?.comment ?? "Congé"}
                </div>
              ) : plannedStart && plannedEnd ? (
                <div className="mt-1.5 rounded-md bg-faraday-50 px-1.5 py-1 text-center text-[11px] font-medium text-faraday-700 sm:text-xs">
                  {plannedStart}–{plannedEnd}
                </div>
              ) : (
                <div className="mt-1.5 text-center text-[11px] text-ardoise-300 sm:text-xs">repos</div>
              )}

              {entry?.actualStart && entry?.actualEnd && !isAbsence && (
                <div className="mt-1 text-center text-[10px] text-ardoise-400 sm:text-[11px]">
                  réel {entry.actualStart}–{entry.actualEnd}
                </div>
              )}
            </>
          );

          if (selectable) {
            return (
              <button
                key={key}
                type="button"
                disabled={isLocked}
                onClick={() => onToggleDate?.(key)}
                className={cellClasses}
                aria-pressed={isSelected}
              >
                {content}
              </button>
            );
          }

          return (
            <div key={key} className={cellClasses}>
              {content}
            </div>
          );
        })}
      </div>

      <div className="mt-4 flex flex-wrap gap-3 text-xs text-ardoise-500">
        <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded bg-faraday-50 ring-1 ring-faraday-200" /> Horaire prévu</span>
        <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded bg-amber-50 ring-1 ring-amber-200" /> Congé</span>
        <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded border border-faraday-500 bg-faraday-50" /> Aujourd&apos;hui</span>
        {selectable && <span className="inline-flex items-center gap-1.5">🔒 Jour verrouillé (mois validé)</span>}
      </div>
    </div>
  );
}
